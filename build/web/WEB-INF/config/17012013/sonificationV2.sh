...
if [ "X${MIDDLEWARE}" = "Xglite" ] ; then
	...
	if [ ! -z $JAVA_HOME ] ; then
		...
		/usr/java/latest/bin/javac Sonification.java
	fi
fi
...
if [ ! -z $JAVA_HOME ] ; then
	echo $JAVA_HOME/bin/java Sonification -f $INPUT_FILE -b $BYTES_SAMPLE -o $SAMPLING -r $RESAMPLE_FREQ -s $SCALE_FACTOR -d $DATA_TYPE -v
	$JAVA_HOME/bin/java Sonification -f $INPUT_FILE -b $BYTES_SAMPLE -o $SAMPLING -r $RESAMPLE_FREQ -s $SCALE_FACTOR -d $DATA_TYPE -v
else
	echo java Sonification -f $INPUT_FILE  -b $BYTES_SAMPLE -o $SAMPLING -r $RESAMPLE_FREQ -s $SCALE_FACTOR -d $DATA_TYPE -v 
	java Sonification -f $INPUT_FILE -b $BYTES_SAMPLE -o $SAMPLING -r $RESAMPLE_FREQ -s $SCALE_FACTOR -d $DATA_TYPE -v
fi
