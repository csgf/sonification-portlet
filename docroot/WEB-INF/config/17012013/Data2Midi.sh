...
mv $INPUT_FILE tungu_tot.dat

echo;echo "- Starting the sonification at ";date
if [ ! -z $JAVA_HOME ] ; then
	echo $JAVA_HOME/bin/java TunguScore
	$JAVA_HOME/bin/java TunguScore
else
	echo /usr/java/latest/bin/java java TunguScore
	/usr/java/latest/bin/java TunguScore
fi
