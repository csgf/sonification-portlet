...
#export CLASSPATH=.:./jargs.jar:./log4j-1.2.15.jar:./jmusic.jar
...
if [ ! -z $JAVA_HOME ] ; then
	echo $JAVA_HOME/bin/javac Text2Midi.java
	$JAVA_HOME/bin/javac Text2Midi.java
else
	echo /usr/java/latest/bin/javac Text2Midi.java
        /usr/java/latest/bin/javac Text2Midi.java
fi
fi
...
# Copying a backup of the input file
cp $INPUT_FILE message2.txt

echo;echo "- Starting the sonification at ";date
if [ ! -z $JAVA_HOME ] ; then
	echo $JAVA_HOME/bin/java Text2Midi -f $INPUT_FILE -m $MULTISCALE -s $SPEED -a $RANGE_MIN -z $RANGE_MAX -v
	$JAVA_HOME/bin/java Text2Midi -f $INPUT_FILE -m $MULTISCALE -s $SPEED -a $RANGE_MIN -z $RANGE_MAX -v
else
	echo /usr/java/latest/bin/java Text2Midi -f $INPUT_FILE -m $MULTISCALE -s $SPEED -a $RANGE_MIN -z $RANGE_MAX -v
        /usr/java/latest/bin/java Text2Midi -f $INPUT_FILE -m $MULTISCALE -s $SPEED -a $RANGE_MIN -z $RANGE_MAX -v
fi

# Renaming the MIDI file
cp messages.mid $INPUT_FILE.mid

if [ $? -eq 0 ] ; then
        echo -n "..Finish at ";date
        echo; echo "Checking results..."
        ls -al $INPUT_FILE.mid
        if [ $? -eq 0 ] ; then
...
#
# README - Sonification
# Sonification Algorithm - ASCIItoMIDI
#
...
~ message.mid:		the audio file produced during the sonification process.
